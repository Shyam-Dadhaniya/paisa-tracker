    ...
    setTimeout(() => {
      try {
        const filtered = expenses.filter((e) => {
          ...
          // Payment mode + sub-source filter
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.div
            key="sheet"
            className="fixed left-0 right-0 bottom-0 z-50 bg-surface rounded-t-2xl flex flex-col shadow-2xl max-h-[90dvh] max-w-md mx-auto"
            initial={{ y: '100%' }}
            animate={{ y: 0 }}
            exit={{ y: '100%' }}
            ...
            <div className="overflow-y-auto flex-1 px-5 py-4 space-y-5">
              ...
              {periodType === 'month' && (
                <div className="flex items-center justify-between bg-surface2 border border-border rounded-xl px-4 py-3">
                  ...
                  <button onClick={nextMonth} className="p-1 text-muted">
